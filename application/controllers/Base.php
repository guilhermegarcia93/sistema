...
class Base extends CI_Controller {
	...
	public function Index()
	{
		...
		$contatos = $this->empresas_model->GetAll('descricao');
		// Passa os contatos para o array que será enviado à home
		$dados['contatos'] =$this->empresas_model->Formatar($contatos);
	...
	public function Salvar(){
		...
		if($validacao){
			...
			$status = $this->empresas_model->Inserir($contato);
	...
	public function Editar(){
		...
		$dados['contato'] = $this->empresas_model->GetById($id);
	...
	public function Atualizar(){
		...
		if($validacao){
			...
			$status = $this->empresas_model->Atualizar($contato['id'],$contato);
			// Checa o status da operação gravando a mensagem na seção
			if(!$status){
				$dados['contato'] = $this->empresas_model->GetById($contato['id']);
	...
	public function Excluir(){
		...
		$status = $this->empresas_model->Excluir($id);
		// Checa o status da operação gravando a mensagem na seção
		if($status){
	...
	*/
	private function Validar($operacao = 'insert'){
		...
		switch($operacao){
			case 'insert':
				$rules['descricao'] = array('trim', 'required', 'min_length[3]');
